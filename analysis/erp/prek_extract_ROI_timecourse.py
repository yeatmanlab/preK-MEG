...
def extract_time_course(cluster_fname):
    ...
    prepost = timepoint.rstrip('Camp')
    hemi_str = cluster_fname.rstrip('.npz')[-2:]
    ...
    for clu in signif_clu:
        ...
        for s in group_members:
            this_subj = os.path.join(data_root, f'{prepost}_camp',
                                     'twa_hp', 'erp', s, 'stc')
            fname = f'{s}FSAverage_{timepoint}_{method}_{condition}'
            ...
            time_course = mne.extract_label_time_course(stc, label,
                                                        ...
                                                        mode='pca_flip',
                                                        allow_empty=True)
            time_courses['s'] = time_course
        df = pd.DataFrame(time_courses, index=stc.times)
        ts_fname = f'{avg_stc_fname}_cluster{clu:05}.csv'
        df.to_csv(os.path.join(ts_dir, ts_fname))
