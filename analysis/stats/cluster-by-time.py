...
rois = ('CoS_lh', 'CoS_rh', 'mFus_lh', 'mFus_rh', 'pFus_lh', 'pFus_rh',
        'IOS_IOG_lh', 'pOTS_lh', 'IOS_IOG_pOTS_lh')

# prepare to load raw data
dtypes = dict(
    time=float,
    value=float,
    roi=pd.StringDtype(),
...
for roi in rois:
    ...
    for contrast, _dict in arrays.items():
        ...
        f_obs, clusters, pvals, H0 = permutation_cluster_test(
            X, threshold=threshold, n_permutations=n_permutations,
            n_jobs=n_jobs, seed=rng, out_type='mask', step_down_p=0.05,
            exclude=exclude)
        cluster_results[roi][contrast] = dict(
            f_obs=f_obs,
            clusters=clusters,
            pvals=pvals,
            H0=H0,
        ...
        if mask.any():
            ...
            with open(f'signif-spans-{roi}-{contrast}.yml', 'w') as f:
                yaml.dump(signif_spans, stream=f, default_flow_style=True)

    # quick plot of the results
    pad = 0.05
    span_kw = dict(color='C1', alpha=0.1)
    fig, axs = plt.subplots(2, 1, sharex=True, sharey=True)
    for contrast, ax in zip(cluster_results[roi], axs):
        pvals = cluster_results[roi][contrast]['pvals']
        ax.plot(times, -np.log10(pvals))
        ...
        for pval in (0.05,):
            ...
            ax.text(times[-1], neglogp - pad, f'p={pval}', va='top',
                    ha='right')
        # significant region
        mask = pvals < 0.05
        if mask.any():
            ...
            ann = (
                'signif.\nregion\n'
                + '-\n'.join(map(str, np.rint(signif_span * 1e3).astype(int)))
                + ' ms')
            ax.annotate(
                ann, xy=(signif_span.mean(), 0.03), va='bottom', ha='center',
                xycoords=ax.get_xaxis_transform(), alpha=0.5,
                fontweight='black', color='C0')

    ax.set_xlabel('time (s)')
    fig.supylabel('\n-log10(p value)\n(post minus pre, cluster-corrected)',
                  ha='center')
    fig.subplots_adjust(left=0.15, right=0.95, hspace=0.4, top=0.88)
    fig.suptitle(f'MPM_{roi}')
    fig.savefig(f'cluster-results-{roi}.png')
# save all results
